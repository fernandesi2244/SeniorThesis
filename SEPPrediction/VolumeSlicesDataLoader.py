...
class SecondarySEPInputDataGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        for row in batch_rows:
            ...
            while len(volume_data) < PrimarySEPInputDataGenerator.TOP_N_BLOBS*(5*nx*ny*channels + 5*nx*nz*channels + 5*ny*nz*channels):
                volume_data.extend(np.zeros(5*nx*ny*channels + 5*nx*nz*channels + 5*ny*nz*channels))
