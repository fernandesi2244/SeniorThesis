...
class ModelConstructor(object):
    ...
    def create_model(cls, dataloader_type, model_type, granularity, n_components, **kwargs):
        ...
        # elif dataloader_type == 'just_cube':
        #     dataloader = JustCubeDataLoader.SecondarySEPInputDataGenerator
        # elif dataloader_type == 'recent_events':
        ...
        model_map = {
            ...
            'nn_simple': lambda: ModelConstructor.get_nn_model(dataloader, granularity, n_components, 'simple', **kwargs),
            'nn_complex': lambda: ModelConstructor.get_nn_model(dataloader, granularity, n_components, 'complex', **kwargs),
            ...
            'conv_nn_on_cube_simple_just_cube': lambda: ModelConstructor.get_conv_nn_just_cube(dataloader, granularity, 'cube_simple'),
    ...
    @staticmethod
    def get_nn_model(dataloader, granularity, n_components, version, **kwargs):
        if n_components == -1:
            if granularity == 'per-blob':
                n_features = kwargs.get('num_features')
                if n_features == -1:
                    one_time_info_size = len(dataloader.BLOB_ONE_TIME_INFO)
                    blobs_size = dataloader.TIMESERIES_STEPS * len(dataloader.BLOB_VECTOR_COLUMNS_GENERAL)
                    complete_input_size = one_time_info_size + blobs_size
                else:
                    complete_input_size = n_features
                    
                if version == 'simple':
                    complete_input_shape = (complete_input_size,)
                    ...
                    # Add input layer
                    layers.append(tf.keras.layers.Input(shape=complete_input_shape))
                    
                    while current_neuron_count > 4:
                        # Calculate neuron count for current layer (next power of 2)
                        neurons = 2 ** (current_neuron_count.bit_length())
                        ...
                        current_neuron_count = current_neuron_count // 2
                    ...
                    inputs = tf.keras.layers.Input(shape=complete_input_shape)
                    ...
                    initial_neurons = min(128, 2 ** (current_neuron_count.bit_length() - 1))
    ...
    def get_conv_nn(dataloader, granularity, version):
        ...
        if version == 'cube':
            ...
            """
            Input format per batch example (all as flattened scalar values immediately following each other)
            - BLOB_ONE_TIME_INFO (11 elements)
            x TOP_N_BLOBS
                -- BLOB_VECTOR_COLUMNS_GENERAL (4 elements)
                -- 5*5*5*channels (5x5x5 cube)
            """
            nx, ny, nz, channels = 200, 400, 100, 3
            ...
            # Process the one-time info with minimal network
            one_time_info_output = tf.keras.layers.Dense(8, activation='relu')(one_time_info_input)
            one_time_info_output = tf.keras.layers.BatchNormalization()(one_time_info_output)

            # Minimal shared layer for blob general info
            general_dense = tf.keras.layers.Dense(6, activation='relu')
            general_bn = tf.keras.layers.BatchNormalization()
            
            # Reduced complexity for cube processing
            # First convolutional block - reduced filters
            cube_conv1 = tf.keras.layers.Conv3D(12, (3, 3, 3), padding='same', activation='relu')
            cube_bn1 = tf.keras.layers.BatchNormalization()
            
            # Second convolutional block - reduced filters
            cube_conv2 = tf.keras.layers.Conv3D(16, (3, 3, 3), padding='same', activation='relu')
            cube_bn2 = tf.keras.layers.BatchNormalization()
            
            # Post-conv dense processing - reduced size
            cube_dense = tf.keras.layers.Dense(20, activation='relu')
            cube_bn_dense = tf.keras.layers.BatchNormalization()
            
            # Combined processing
            combined_dense = tf.keras.layers.Dense(12, activation='relu')
            combined_bn = tf.keras.layers.BatchNormalization()
            ...
            for i in range(dataloader.TOP_N_BLOBS):
                ...
                # Process cube with simplified convolutional architecture
                cube = tf.keras.layers.Reshape((5, 5, 5, channels))(cube)
                ...
                cube_out = tf.keras.layers.MaxPooling3D(pool_size=(2, 2, 2), padding='same')(cube_out)
                
                # Second convolutional layer
                cube_out = cube_conv2(cube_out)
                cube_out = cube_bn2(cube_out)
                
                # Flatten and process with dense layer
                cube_out = tf.keras.layers.Flatten()(cube_out)
                cube_out = cube_dense(cube_out)
                cube_out = cube_bn_dense(cube_out)
            ...
            # Combine blob outputs with simple max pooling
            ...
            x = tf.keras.layers.Dense(12, activation='relu')(combined_output)
            x = tf.keras.layers.BatchNormalization()(x)
    ...
    def get_conv_nn_just_cube(dataloader, granularity, version):
    ...
    def get_conv_nn_recent_events(dataloader, granularity, version):
        # NOTE: Function not completely implemented, used as a quick robustness check.
        # Please refer to get_conv_nn for full functionality.
        if granularity == 'per-blob':
            raise ValueError('Convolutional models are not yet supported for per-blob granularity')
