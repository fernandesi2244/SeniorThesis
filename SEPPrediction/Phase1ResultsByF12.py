...
print("\nHarmonic Mean and Average of F-0.5 Scores for Each Granularity:")
for i, (df, name) in enumerate(zip(results_dfs, results_names)):
    print(f"\n{name.capitalize()} Data:")
    granularity_types = df['granularity'].unique()
    
    for granularity in granularity_types:
        f_half_scores = df[df['granularity'] == granularity]['f_half']
        if len(f_half_scores) > 0:
            harmonic_mean_f_half = len(f_half_scores) / np.sum(1.0 / f_half_scores)
            average_f_half = f_half_scores.mean()
            print(f"Granularity: {granularity}, Harmonic Mean of F-0.5 Scores: {harmonic_mean_f_half:.4f}, Average of F-0.5 Scores: {average_f_half:.4f}")
        else:
